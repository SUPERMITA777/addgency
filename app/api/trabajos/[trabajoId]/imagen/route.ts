...
import { applyImageWatermark } from '@/lib/watermark/image';
...
export async function GET(
  ...
  try {
    ...
    if (trabajo.marcaDeAgua) {
      ...
      outputBuffer = await applyImageWatermark(fileBuffer, watermarkText);
    }

    // Detect MIME type based on file extension
    let mimeType = 'image/jpeg';
    if (trabajo.archivoPath.endsWith('.png')) mimeType = 'image/png';
    else if (trabajo.archivoPath.endsWith('.webp')) mimeType = 'image/webp';
    else if (trabajo.archivoPath.endsWith('.gif')) mimeType = 'image/gif';
